...
local function equipment_grid_()
	return T.grid{
		T.row { T.column { T.label { id = "the_gearlist_title" }}},
		T.row { T.column { vertical_grow = false, horizontal_alignment = "left" , horizontal_grow = false, T.horizontal_listbox { horizontal_scrollbar_mode = "never", id = "the_gearlist" , T.list_definition { T.row { T.column {horizontal_grow = true,
					T.toggle_panel { T.grid {
								T.row { T.column { T.image { id = "the_gearlist_icon"}}},
								T.row { T.column { T.label { id = "the_gearlist_icon_name"}}}
								}}
			}}}}}},
...
function set_simple_grid_values(unit)
	...
	wesnoth.set_dialog_value(string.format("%s~SCALE(400,400)", unit.profile) , "the_image")
