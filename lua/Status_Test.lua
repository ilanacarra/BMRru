           ...
           T.row { T.column { T.grid {
			           T.row { 
			           		T.column {  horizontal_alignment = "left" , T.grid { --left grid
			    	   	   	...
			    	   	   	T.column {vertical_alignment = "bottom" , T.grid { --right grid
...
local function preshow()
    ...
    wesnoth.set_dialog_value(string.format("<span size='xx-large' color='#eeffb7'> %s </span>", unit_cfg.name) , "the_title")
...
-- calling from inventory - no button yet
	  elseif rv == 6 then
	    call_from_pool(rv,pli)
-- delete from inventory
	  elseif rv == 5 then
	    delete_from_pool(rv,pli)
  	  end
